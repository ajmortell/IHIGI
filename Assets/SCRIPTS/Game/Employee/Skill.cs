using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class Skill : MonoBehaviour {



    public int writing = 0;
    public int drawing = 0;
    public int building = 0;
    public int arithmetic = 0;
    public int comunication = 0;
    public int enforcing = 0;
    public int detecting = 0;
    public int reading = 0;
    public int logic = 0;
    public int serving = 0;
    public int study = 0;
    public int communication = 0;
    public int pursuasion = 0;
    public int accuracy = 0;
    public int adaptability = 0;
    public int analytics = 0;
    public int attention = 0;
    public int creativity = 0;
    public int editing = 0;
    public int comprehension = 0;
    public int counseling = 0;
    public int criticalThinking = 0;
    public int defending = 0;
    public int drama = 0;
    public int driving = 0;
    public int ethics = 0;
    public int evaluation = 0;
    public int expression = 0;
    public int punctuality = 0;
    public int gathering = 0;
    public int leadership = 0;
    public int handling = 0;
    public int inspecting = 0;
    public int listening = 0;
    public int maintenace = 0;
    public int management = 0;
    public int motivator = 0;
    public int navigation = 0;
    public int negotiation = 0;
    public int multiTasking = 0;
    public int operation = 0;
    public int patience = 0;
    public int organization = 0;
    public int performing = 0;
    public int planning = 0;
    public int prediction = 0;
    public int potential = 0;
    public int stamina = 0;
    public int cooking = 0;
    public int relaxation = 0;
    public int painting = 0;
    public int plumbing = 0;
    public int mechanic = 0;
    public int healing = 0;
    public int caring = 0;
    public int sense = 0;
    public int curiosity = 0;
    public int friendship = 0;
    public int perseverance = 0;
    public int responsible = 0;
    public int humor = 0;
    public int dancing = 0;
    public int rhythm = 0;
    public int dependability = 0;
    public int emotion = 0;
    public int interpersonal = 0;
    public int adept = 0;
    public int acute = 0;
    public int ambition = 0;
    public int assertion = 0;
    public int competance = 0;
    public int cooperation = 0;
    public int deduction = 0;
    public int dedication = 0;
    public int diplomacy = 0;
    public int empathy = 0;
    public int innovation = 0;
    public int intuition = 0;
    public int loyalty = 0;
    public int nonverbal = 0;
    public int passion = 0;
    public int perception = 0;
    public int social = 0;
    public int teaching = 0;
    public int tracking = 0;

    List<int> skillsList;

    public void increaseStat(int amt) {

    }

    public void decreaseStat(int amt) {

    }

    void Awake() {

    }
    
    void Start () {
		
	}
	
	void Update () {
		
	}
}
